 ...
 * 基金投资顾问 Mastra Agent
 *
 * 真正基于 Mastra Core 框架实现
 * 使用 AI SDK 的 OpenAI 兼容接口连接智谱AI (GLM)
 */

import { createTools } from '@mastra/core';
...
export const analyzePortfolioTool = {
  ...
  execute: async ({ funds }: { funds: string }) => {
 ...
 * 工具4: 基金深度分析
 */
export const analyzeFundDeeplyTool = {
  ...
  execute: async ({ fundCode, fundName }: { fundCode: string; fundName: string }) => {
    const theories = [
      {
        name: '现代投资组合理论 (MPT)',
        score: 65,
 ...
 * 工具5: 搜索基金资料
 */
export const searchFundResearchTool = {
 ...
 * 工具6: 使用特定理论分析
 */
export const analyzeFundWithTheoryTool = {
 ...
 * 工具7: 执行完整分析工作流
 */
export const runFundAnalysisWorkflowTool = {
